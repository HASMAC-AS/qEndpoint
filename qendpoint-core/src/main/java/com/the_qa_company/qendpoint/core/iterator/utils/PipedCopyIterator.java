...
public class PipedCopyIterator<T> implements Iterator<T>, Closeable {
	...
	private final SpscArrayQueue<QueueObject<T>> activeReadQueue;
	private final SpscArrayQueue<QueueObject<T>> activeWriteQueue;
	private final SpscArrayQueue<Object[]> recycledArrays;
...
//		if(queue2.peek().end()) queue2.poll();
	...
	public PipedCopyIterator(int batchSize) {
		...
		this.activeReadQueue = new SpscArrayQueue<>(queueCapacity);
		this.activeWriteQueue = this.activeReadQueue;
		this.recycledArrays = new SpscArrayQueue<>(nextPowerOfTwo(queueCapacity * 2));
	}
}
