...
* ✅ Use `-am` **only** for compile/verify with tests skipped (e.g. `-DskipTests`):
    * `mvn -o -Dmaven.repo.local=.m2_repo -pl <module> -am -DskipTests clean install`
...
* Always run `mvn -T 1C -o -Dmaven.repo.local=.m2_repo -DskipTests clean install | tail -200` before you start working. This command typically takes up to 30 seconds. Never use a shorter timeout than 60,000 ms.
* Always run `mvn -T 1C -o -Dmaven.repo.local=.m2_repo -DskipTests clean install | tail -200` before any `verify` or test runs.
* If offline resolution fails due to a missing dependency or plugin, run the command without `-o`: `mvn -Dmaven.repo.local=.m2_repo -DskipTests clean install | tail -200`, then return offline.
* If it fails for any other reason, run the command without `-T 1C`: `mvn -o -Dmaven.repo.local=.m2_repo -DskipTests clean install | tail -200`.
...
* **Build without tests (fast path):**
  `mvn -T 1C -o -Dmaven.repo.local=.m2_repo -DskipTests clean install`
* **Verify with tests:**
